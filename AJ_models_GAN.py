...
class learning_gan:
    ...
    def summarize_performance(self, data, epoch, generator, discriminator, latent_dim, verbose=False, save_plot = False):
        '''
        ...
        x_fake, y_fake = self.generate_fake_samples(generator, latent_dim, n_generate)
    ...
    def train(self, data, latent_dim, n_epochs=10000, n_batch=128, n_eval=2000, verbose = False, save = False, save_plot = False):
        '''
        training function.
        data: training set in formato pandas
        latent_dim: (int) dimensione del vettore dello spazio latente, deve essere lungo quanto il primo layer del modello
        ...
        if n_batch>data.shape[0]:
            n_batch = data.shape[0]
        half_batch = int(n_batch / 2)
        sum_epoch, sum_acc_real, sum_acc_fake = [], [], []
        sum_d_loss_real, sum_d_loss_fake, sum_g_loss = [], [], []
        # manually enumerate epochs
        pbar = tqdm(range(n_epochs), position = 0, leave = True, ascii = True, unit='epochs')
        for i in pbar:
            # prepare real samples
            ...
            if (i+1) % n_eval == 0:
                temp_epoch, temp_real, temp_fake = self.summarize_performance(data, i, generator, discriminator, latent_dim, verbose, save_plot)
                sum_epoch.append(temp_epoch)
                sum_acc_real.append(temp_real)
                sum_acc_fake.append(temp_fake)
                sum_d_loss_real.append(d_loss_real[0].tolist())
                sum_d_loss_fake.append(d_loss_fake[0].tolist())
                sum_g_loss.append(g_loss.tolist())
                if save:

...
class learning_class:
    ...
    def train_models(self, models, xtrain, ytrain, epochs = None, validation_data = None, shuffle = True, batch_size = 32):
        ...
        for i, (name_model, model) in enumerate(models.items()):
            ...
            if name_model == 'deep learning vgg sparse' or name_model == 'deep learning vgg binary':
                xvgg = xtrain.copy()
                xvgg = xvgg.to_numpy()
            ...
            if name_model == 'XGBClassifier' and validation_data:
    ...
    def score_accuracy_recall(self, y_pred_matrix, y_true, verbose = 1):
        ...
        for col in y_pred_matrix.columns:
            ...
            prob_cm = np.zeros((len(cm[:,0]), len(cm[0,:])))
            ...
            for i in range(len(cm[:,0])):
                ...
                tot_true_val.append(cm[i,:].sum())
                tot_predic_val.append(cm[:,i].sum())
                if tot_true_val[i] == 0:
                    recall.append(0)
                else:
                    recall.append(cm[i,i]/tot_true_val[i])
                if tot_predic_val[i] == 0:
                    precision.append(0)
                else:
                    precision.append(cm[i,i]/tot_predic_val[i])
            ...
            df_predict['recall'] = df_cm['recall'].sum()/len(recall)
            df_cm = pd.concat([df_cm, df_predict])

            df_precision = pd.DataFrame(precision, columns = ['precision']).T
            df_precision['tot true'] = df_precision.loc['precision'].sum()/len(precision)

            accuracy_tot = 0
            accuracy_average_tot = 0

            for i in range(len(cm[:,0])):
                accuracy_tot = cm[i,i] + accuracy_tot
                accuracy_average_tot = prob_cm[i,i]+accuracy_average_tot
            accuracy_average_tot = accuracy_average_tot/len(cm[:,0])
            accuracy_tot = accuracy_tot/len(y_true)

            df_precision['recall'] = accuracy_tot
            df_cm= pd.concat([df_cm, df_precision])
            # st.write(df_cm)

            accurary_real[col] = [accuracy_tot, accuracy_average_tot, df_cm['tot true']['precision']]
            if verbose == 1:
                print(col)
                fig, ax = plot_confusion_matrix(conf_mat=cm, show_normed=True, show_absolute=True, colorbar=True, class_names=labels)
                plt.show()
                sns.heatmap(df_cm, annot=True, annot_kws={'size': 8}, cmap=plt.cm.Blues, vmax=tot_samples, vmin=0, square=True, linewidths=0.1, linecolor="black")
                plt.show()

        accurary_real = accurary_real.T
        accurary_real.columns = ['accuracy_tot', 'recall_average', 'precision_average']
